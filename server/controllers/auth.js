...
exports.requireSignin = expressJwt({
    ...
    exports.forgotPassword = (req, res) => {
        ...
        User.findOne({email}, (err, user) => {
            if(err || !user) {
                return res.status(400).json({
                    error: 'User with that email does note exist'
                });
            }
            ...
            return user.updateOne({resetPasswordLink: token}, (err, success) => {
                if(err) {
                    ...
                    return res.status(400).json({
                        error: 'Database connection error on user password for request'
                    });
                } else {
                    sgMail
                        .send(emailData)
                        .then(sent => {
                            ...
                            return res.json({
                                message: `Email has been sent to ${email}. Follow the instruction for forgot password`
                            });
                        })
                        .catch(err => {
        ...
        if(resetPasswordLink) {
            jwt.verify(resetPasswordLink, process.env.JWT_RESET_PASSWORD, function(err, decoded) {
                ...
                User.findOne({resetPasswordLink}, (err, user) => {
                    if(err || !user) {
                        return res.status(400).json({
                            error: 'Somethign went wrong. Try later'
                        })
                    }
